...
class TestWeeklyGames(unittest.TestCase):
    ...
    def _test_game_ratings(self, uri):
        match uri:
            case '202211100car': self.assertEqual(self.games.weekly_ratings[uri], 1.5, "Wrong rating for car")
            case '202211130tam': self.assertEqual(self.games.weekly_ratings[uri], 5.5, "Wrong rating for tam")
            case '202211130buf': self.assertEqual(self.games.weekly_ratings[uri], 17, "Wrong rating for buf")
            case '202211130chi': self.assertEqual(self.games.weekly_ratings[uri], 11.5, "Wrong rating for chi")
            case '202211130kan': self.assertEqual(self.games.weekly_ratings[uri], 8.5, "Wrong rating for kan")
            ...
            case '202211130rai': self.assertEqual(self.games.weekly_ratings[uri], 7.5, "Wrong rating for rai")
            case '202211130gnb': self.assertEqual(self.games.weekly_ratings[uri], 11, "Wrong rating for gnb")
            case '202211130ram': self.assertEqual(self.games.weekly_ratings[uri], 2, "Wrong rating for ram")
            case '202211130sfo': self.assertEqual(self.games.weekly_ratings[uri], 6, "Wrong rating for sfo")
            case '202211140phi': self.assertEqual(self.games.weekly_ratings[uri], 2.5, "Wrong rating for phi")
...
class TestSingleGame(unittest.TestCase):
    ...
    def test_game(self):
        self.assertEqual(self.game, 17, "Wrong rating for buf game")
